...
document
	...
	?.addEventListener("submit", function (event) {
		...
		const candidateData = {
			cname: document.getElementById("candidateName").value,
			pname: document.getElementById("party").value,
		};

		fetch("http://localhost:8081/api/candidate/register", {
			method: "POST",
			headers: {
		...
		.then((candidate) => {
			Swal.fire({
				...
				html: `<strong>ID:</strong> ${candidate.id} <br>
                       <strong>Name:</strong> ${candidate.cname} <br>
                       <strong>Party:</strong> ${candidate.pname}`,
				icon: "info",
			});
		})
...
function updateCandidate(candidateId, newName, newParty) {
	fetch(`http://localhost:8081/api/candidate/update/${candidateId}`, {
		method: "PUT",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({ cname: newName, pname: newParty }),
	})
		.then(() => {
			showToast("Candidate updated successfully!", "success");
			loadCandidates();
